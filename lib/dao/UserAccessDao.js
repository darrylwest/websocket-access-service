    ...
    this.authenticate = function(request, callback) {
        ...
        var userFoundCallback = function(err, user) {
            ...
            if (dao.calculateDigest) {
                var hash = dao.calculateDigest( user.accessKey, request.token),
                    status = (hash === request.hash ? 'ok' : 'failed');

                log.info('hash: ', hash);
                if (status !== 'ok') {
                    log.warn('request hash: ', request.hash);
                    log.warn('calcltd hash: ', hash);
                }
